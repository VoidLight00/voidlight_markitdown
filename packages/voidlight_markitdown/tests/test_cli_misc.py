...
import subprocess
from voidlight_markitdown import __version__
...
def test_version() -> None:
    result = subprocess.run(
        ["python", "-m", "voidlight_markitdown", "--version"], capture_output=True, text=True
    )
...
def test_invalid_flag() -> None:
    result = subprocess.run(
        ["python", "-m", "voidlight_markitdown", "--foobar"], capture_output=True, text=True
    )
