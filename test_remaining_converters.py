#!/usr/bin/env python3
"""
Comprehensive test suite for remaining converters:
1. Audio converter (with actual audio files)
2. EPUB converter (with actual EPUB files)
3. Outlook MSG converter
4. YouTube transcript converter
5. RSS feed converter
...
<body>
    ...
    def create_test_msg_file(self, filename="test.msg"):
        """Create a simple MSG file structure for testing"""
        filepath = os.path.join(self.temp_dir, filename)
        
        # Note: Creating a proper MSG file is complex as it uses OLE format
        # For testing purposes, we'll create a minimal OLE file structure
        # This is a simplified version and may not work with all MSG parsers
        ...
        try:
            ...
            # Add the required streams for MSG format
            # Subject
            subject_data = "Test Email Subject\x00".encode('utf-16-le')
            ole._add(ole._find_create_stream("__substg1.0_0037001F"), subject_data)
            
            # From
            from_data = "test@example.com\x00".encode('utf-16-le')
            ole._add(ole._find_create_stream("__substg1.0_0C1F001F"), from_data)
            
            # To
            to_data = "recipient@example.com\x00".encode('utf-16-le')
            ole._add(ole._find_create_stream("__substg1.0_0E04001F"), to_data)
            
            # Body
            body_data = "This is a test email body.\n\nIt has multiple lines.\x00".encode('utf-16-le')
            ole._add(ole._find_create_stream("__substg1.0_1000001F"), body_data)
            
            # Properties version
            ole._add(ole._find_create_stream("__properties_version1.0"), b'\x00' * 8)
            
            # Recipient version
            ole._add(ole._find_create_stream("__recip_version1.0_#00000000"), b'\x00' * 8)
            ...
            # If olefile is not available or has issues, create a mock file
            # This creates a minimal OLE file that should be recognized
            with open(filepath, 'wb') as f:
                # Write OLE header signature
                f.write(b'\xd0\xcf\x11\xe0\xa1\xb1\x1a\xe1')
                # Write minimal header data
                f.write(b'\x00' * 22)  # Reserved
                f.write(b'\x00\x03')    # Minor version
                f.write(b'\x00\x03')    # Major version (version 3)
                f.write(b'\xfe\xff')    # Byte order identifier
                f.write(b'\x00\x09')    # Sector shift (512 bytes)
                f.write(b'\x00\x06')    # Mini sector shift
                f.write(b'\x00' * 6)    # Reserved
                f.write(b'\x00' * 4)    # Total sectors
                f.write(b'\x00' * 4)    # FAT sectors
                f.write(b'\x00' * 4)    # First directory sector
                f.write(b'\x00' * 4)    # Transaction signature
                f.write(b'\x00\x10\x00\x00')  # Mini stream cutoff size
                f.write(b'\xff\xff\xff\xff')  # First mini FAT sector
                f.write(b'\x00' * 4)    # Number of mini FAT sectors
                f.write(b'\x01\x00\x00\x00')  # First DIFAT sector
                f.write(b'\x00' * 4)    # Number of DIFAT sectors
                # Write DIFAT array (109 entries)
                for i in range(109):
                    f.write(b'\xff\xff\xff\xff')
                # Pad to 512 bytes
                current_size = f.tell()
                if current_size < 512:
                    f.write(b'\x00' * (512 - current_size))
...
<body>
    ...
    def test_msg_converter(self):
        ...
        try:
            # Test 1: MSG file
            msg_file = self.create_test_msg_file()
    ...
    def generate_report(self):
        ...
        report_file = os.path.join(os.path.dirname(__file__), "test_remaining_converters_report.json")
